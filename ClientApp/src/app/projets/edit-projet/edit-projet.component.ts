...
export class EditProjetComponent implements OnInit {
  ...
  constructor(private router: Router, private _projetService: ProjetService,
  ...
  ngOnInit() {
    ...
    this._projetService.getProjectById(this.id)
    .subscribe( data => {
...
onSubmitProject() {
  ...
  this._projetService.UpdateProject
    (this.ProjectForm.value, this.id)
    .pipe(first())
    .subscribe(
      data => {
